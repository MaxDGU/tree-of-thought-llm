...
log_file_path = "llama_game24_output.log"
...
class Tee(io.TextIOBase):
    ...
    def write(self, data):
        ...
        self.stream1.flush() # Flush file buffer
        self.stream2.flush() # Flush console buffer
    def flush(self):
        self.stream1.flush()
        self.stream2.flush()
...
try:
    ...
    try:
        from tot.methods.bfs import solve # Import solve again
    ...
    args = argparse.Namespace(
        backend='meta-llama/Llama-3.2-1B',  
        temperature=0.7,
        task='game24',
        naive_run=False,
        prompt_sample=None, 
    ...
    try:
        ys, infos = solve(args, task, idx, to_print=True) # Ensure to_print is True for logs
        ...
        # print(f"Solve duration before error: {end_solve_time - start_solve_time:.2f} seconds.")

    print(f"\n{time.time():.2f} - Main script logic finished.")

finally:
    # --- Restore Output Streams and Close File ---
    # Check if redirection happened before trying to restore/close
